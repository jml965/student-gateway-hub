...
`;
  ...
  if (facts.length > 0) {
    memorySection = `
## معلومات الطالب من محادثات سابقة (استخدمها فقط إذا ذكر الموضوع — لا تحشرها في التحيات)
${facts.map((f) => `- ${f}`).join("\n")}
`;
  }
...
`;
  ...
  // ── خريطة الخدمة التالية بناءً على السياق ──
  const serviceDescriptions: Record<string, { label: string; card: string; pitch: string }> = {
    housing:      { label: "السكن الطلابي",        card: "[CARD:housing]",      pitch: "وبما أنك ستلتحق بالجامعة، هل فكّرت في السكن؟ نوفر سكناً مفروشاً قريباً منها من 150$/شهر." },
    insurance:    { label: "التأمين الصحي",        card: "[CARD:insurance]",    pitch: "بعد ترتيب السكن، التأمين الصحي خطوة ضرورية — تغطية شاملة في 40+ دولة من 30$/شهر." },
    airport_pickup:{ label: "الاستقبال من المطار",  card: "[CARD:airport_pickup]",pitch: "هل ترتّبت لوصولك؟ نرسل لك شخصاً يستقبلك في المطار ويوصلك بأمان — في 80+ مطار." },
    visa:         { label: "تجهيز الفيزا",          card: "[CARD:visa]",         pitch: "نساعدك في تجهيز ملف التأشيرة بالكامل — وثائق، حجوزات، وحجز موعد السفارة." },
    internet:     { label: "الإنترنت الدولي",       card: "[CARD:internet]",     pitch: "شريحة SIM دولية تعمل في 150+ دولة من 15$/شهر — تبقى متصلاً بعائلتك." },
    student_card: { label: "البطاقة الطلابية ISIC", card: "[CARD:student_card]", pitch: "بـ 20$/سنة تحصل على 150,000+ خصم في المطاعم والمواصلات والمتاجر حول العالم." },
    prepaid_card: { label: "بطاقة Visa/Mastercard", card: "[CARD:prepaid_card]", pitch: "بطاقة مسبقة الدفع للطلاب الدوليين تحل مشكلة الدفع في الخارج — تصلك سريعاً." },
    travel:       { label: "حجز تذاكر السفر",       card: "[CARD:travel]",       pitch: "خصومات طيران للطلاب تصل 30% — أرخص رحلة إلى جامعتك." },
  };

  // الخدمة المنطقية التالية بناءً على مرحلة الطالب
  const stageNextService: Record<string, string> = {
    cold:       "housing",
    exploring:  "housing",
    interested: "housing",
    comparing:  "housing",
    decided:    "housing",
    applied:    "airport_pickup",
    enrolled:   "insurance",
  };

  const currentStage = memory.stage ?? "exploring";
  const nextServiceKey = stageNextService[currentStage] ?? "housing";
  const nextService = serviceDescriptions[nextServiceKey];

  const stageInstructions: Record<string, string> = {
    cold:       "اسأل سؤالاً واحداً لفهم هدفه. لا تعرض خدمات بعد.",
    exploring:  "اسأل عن التخصص أو البلد أو الميزانية. خطوة واحدة فقط.",
    interested: "قدّم له جامعة أو خيارين محددين. لا تزد.",
    comparing:  "ساعده في المقارنة بين خيارين فقط. اطرح سؤالاً يوجّهه للقرار.",
    decided:    "وجّهه نحو إتمام التسجيل مباشرة. الأمر سهل ونحن نساعده.",
    applied:    "احتفل معه ثم اقترح خدمة الاستقبال من المطار فقط.",
    enrolled:   "اسأله عن احتياجاته الآن — التأمين أو الإنترنت.",
  };

  const stageGuide = stageInstructions[currentStage] ?? stageInstructions["exploring"];
...
## مرحلة الطالب الحالية: ${currentStage}
${stageGuide}
${studentProfile.hasApplication ? "✅ لديه طلب تسجيل مسبق." : "⚡ لم يتقدم بعد — وجّهه نحو التسجيل في الوقت المناسب."}

## قانون التسويق التدريجي — القاعدة الأهم
❌ **ممنوع منعاً باتاً**: ذكر أكثر من خدمة واحدة في نفس الرسالة.
❌ **ممنوع**: عرض قائمة بالخدمات أو شرح كل ما تقدمه Baansy دفعة واحدة.
✅ **الأسلوب الصحيح**:
1. أجب على سؤال المستخدم بالكامل أولاً
2. في نهاية الرد فقط — اقترح الخدمة التالية المنطقية **بجملة واحدة**
3. انتظر ردّه قبل أن تنتقل للخدمة التالية

## الخدمة التالية المقترحة الآن
بناءً على مرحلته، الخدمة الطبيعية التالية هي: **${nextService.label}**
إذا كان السياق مناسباً، أضف في نهاية ردك هذه الجملة فقط:
"${nextService.pitch}" ${nextService.card}

## قاموس خدمات Baansy (للرجوع عند الحاجة فقط)
- السكن: سكن مفروش قريب من الجامعة، 150$/شهر [CARD:housing]
- التأمين: تغطية طبية شاملة، 40+ دولة، 30$/شهر [CARD:insurance]  
- الاستقبال: شخص يستقبلك في المطار، 80+ مطار [CARD:airport_pickup]
- الفيزا: تجهيز كامل للملف [CARD:visa]
- الإنترنت: SIM في 150+ دولة، 15$/شهر [CARD:internet]
- البطاقة الطلابية ISIC: 150,000+ خصم حول العالم [CARD:student_card]
- بطاقة Visa/Mastercard مسبقة الدفع [CARD:prepaid_card]
- حجز تذاكر بخصم طلابي 30% [CARD:travel]

## متطلبات التأشيرة (للرجوع عند السؤال فقط)
${visaSummary}

## قواعد الرد
- ردودك **قصيرة وبشرية** — 2-4 أسطر كحد أقصى
- تجاوب **بلغة المستخدم** (عربي أو إنجليزي)
- **لا تذكر منافسين** بالاسم
- **رموز CARD**: خدمة واحدة فقط في نهاية الرد، ليس في المنتصف
- إذا سأل عن شيء خارج نطاق Baansy، أجب بإيجاز وأعِد التوجيه بلطف
`;
}
